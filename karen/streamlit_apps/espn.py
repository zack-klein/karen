...
def build_app(
    ...
    st.beta_set_page_config(
        page_title=APP_TITLE, page_icon=FAVICON, layout="wide"
    )
    ...
    *These power rankings are calculated using a two-step dominance matrix (you
    ...
    full_league.build_player_summary_df(
        week=for_weeks_players, on_teams=on_teams
    )
    ...
    position_stats_type = st.selectbox(
        "Pick a statistic:", position_stats_types
    )
    ...
    st.write(
        ...
        "- [Fantasy Pros](https://www.fantasypros.com/nfl/start/) would start the "  # noqa:E501
        "the FA"
    )
